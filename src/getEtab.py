...
# https://data.education.gouv.fr/explore/dataset/fr-en-annuaire-education/api/ (OK ! @author: Bobby)
...
def GetEtablissements(win):
    ...
    #===========================================================================
    # GetEtablissementsParType
    #===========================================================================
    def GetEtablissementsParType(url, type, liste):
         
        print("\nRécupération des " + type + " depuis l'annuaire de l'Education Nationale")
        urlResponse = requests.get(url)
        if urlResponse.ok :
            print("Requête réalisée avec succès")
            parsedData = json.loads(urlResponse.text)
            print(str(parsedData["nhits"]) + " " + type + " trouvés")
            print("Ajout des " + type + " à la liste des établissements") 
            for i in range(parsedData["nhits"]) :
                etablissement = []
                etablissement.append(parsedData["records"][i]["fields"]["libelle_academie"])
                etablissement.append(parsedData["records"][i]["fields"]["type_etablissement"])
                etablissement.append(parsedData["records"][i]["fields"]["nom_etablissement"])
                etablissement.append(parsedData["records"][i]["fields"]["nom_commune"])
                liste.append(etablissement)
        else :
            print("Erreur! Impossible de récupérer les données des " + type)
            print("Vérifiez votre connexion à Internet.")       
        
        print(str(len(liste)) + " établissements importés")
...
Paramètres :
    elem : objet de type ElementTree
...
def SauvEtablissements(win, path):
    ...
    #Obtention de la liste d'établissements
    listeEtablissements = GetEtablissements(win)
    
    #Vérification de la liste
    if len(listeEtablissements) > 0:
        nomF = os.path.join(path, "Etablissements.xml")

        # Conversion de la liste en arbre (XML)
        ...
        for i in range(1,len(listeEtablissements)):
         ...
         #Indentation du fichier puis écriture
        indent(root)
        ET.ElementTree(root).write(nomF, encoding = "utf-8")
    #         fichier.close()
